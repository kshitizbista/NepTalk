...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if Auth.auth().currentUser != nil {
            //  instantiate the main tab bar controller and set it as root view controller using the storyboard identifier
            let mainTabBarController = storyboard.instantiateViewController(identifier: "MainTabBarController")
            window?.rootViewController = mainTabBarController
        } else {
            // if user isn't logged in login instantiate the navigation controller and set it as root view controller using the storyboard identifier
